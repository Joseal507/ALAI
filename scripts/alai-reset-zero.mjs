...
function curriculumJob(stage, target, priority, path) {
  addJob.run({
    id:id('job'),
    topic:`ALAI_CURRICULUM_EXPAND | stage=${stage} | path=${path} | target=${target} | task=Build full academic map with courses, units, topics, subtopics, prerequisites, exercises, assessments, Panama context when relevant, and universal standards.`,
    priority,
    created_at:now,
    updated_at:now,
    stage,
    base_priority:priority,
    source:'curriculum_os'
  });
}
...
for (const [stage, priority] of stages) node(stage, 'stage', stage, stage, priority);
...
for (const [stage, subjects] of Object.entries(school)) {
  const priority = stages.find(s => s[0] === stage)[1];
  const root = node(stage, 'stage', stage, stage, priority);
  for (const subject of subjects) {
    const path = `${stage}/${subject}`;
    node(stage, 'subject', subject, path, priority, root);
    curriculumJob(stage, subject, priority, path);
  }
}
...
VALUES ('main','PREESCOLAR','education_first',1,?)
ON CONFLICT(id) DO UPDATE SET
active_stage='PREESCOLAR',
...
console.log('✅ ALAI reset desde cero con Curriculum OS medible.');
